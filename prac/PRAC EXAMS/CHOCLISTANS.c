...
openDic convertToOpenDict(cursorSet *A) {
    ...
    for (x = 0; x != A->count; ++x) {
        ...
        trav = &newDic.header[hashValue]; 

        for (;*trav != -1; trav = &newDic.dicVHptr->VH_node[*trav].next) {}
        if (*trav == -1) {
            int avail = mallocInVHeap(newDic.dicVHptr);
            if (avail != -1) {
                newDic.dicVHptr->VH_node[avail].elem = A->VHptr->VH_node[heapVal].elem;
                newDic.dicVHptr->VH_node[avail].next = *trav;
                heapVal = A->VHptr->VH_node[avail].next;
                *trav = avail;
